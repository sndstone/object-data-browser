...
import 'package:s3_browser_crossplat/services/ios_engine_service.dart';
import 'package:s3_browser_crossplat/services/engine_service.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  const channel = MethodChannel('s3_browser_crossplat/ios_engine');
  ...
  test('listBuckets returns platform data instead of placeholder fallback',
    ...
    final service = IosEngineService(channel: channel);
    final buckets =
  ...
  test('listBuckets accepts iOS object-keyed platform maps', () async {
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(channel, (call) async {
      if (call.method == 'dispatch') {
    ...
    final service = IosEngineService(channel: channel);
    final buckets =
        await service.listBuckets(engineId: 'ios', profile: profile());
  ...
  test('emits structured iOS API traces when diagnostics are enabled',
      () async {
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(channel, (call) async {
      if (call.method == 'dispatch') {
      ...
      if (call.method == 'dispatch') {
        return {
          'error': {
            ...
            'message': 'Not implemented on iOS.',
          },
        };
      }
      return null;
    ...
    await expectLater(
      ...
      throwsA(
        isA<EngineException>()
            ...
            .having(
                (error) => error.message, 'message', 'Not implemented on iOS.'),
      ),
    );
  });
}
